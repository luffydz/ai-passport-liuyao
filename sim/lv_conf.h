...
/* ★ 分配器选择 —— 与真机对齐的关键一项。
 *
 * 官方实录 docs/reference/y2lin/meter-ui-smoothing-and-layout.md 证实：
 * 24KB 内置静态池在建规模稍大的界面时会耗尽（约 90 对象 / 约 35KB 峰值），
 * lv_obj_create() 返回 NULL，后续 style 调用解引用它 -> 白屏（没有崩溃日志）。
 * 官方的正解是切到 C 库分配器，即真机的 CONFIG_LV_USE_CLIB_MALLOC=y。
 *
 * 所以模拟器默认也用 CLIB。要复现那个失败场景：
 *   cmake -S . -B build -DSIM_LV_BUILTIN_POOL=1
 */
#ifndef SIM_LV_BUILTIN_POOL
    #define SIM_LV_BUILTIN_POOL 0
#endif
#if SIM_LV_BUILTIN_POOL
    #define LV_USE_STDLIB_MALLOC    LV_STDLIB_BUILTIN
#else
    /* CUSTOM 的实现见 sim/host/lv_alloc_counter.c：
     * 行为与 CLIB 完全一致（直接 malloc/free/realloc），只是额外做了精确计数，
     * 所以既能代表真机的 CLIB 配置，又能拿到可信的 LVGL 占用数字。 */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /* ★ 与真机对齐：sdkconfig 的 CONFIG_LV_MEM_SIZE_KILOBYTES=24。
     * LVGL v9 默认 64KB，这里压到 24KB，池子大小与真机一致，
     * 才能提前暴露"池不够导致绘制失败"的问题。 */
    /* 仅在 SIM_LV_BUILTIN_POOL=1（内置池模式）时生效。
     * 默认取真机的 24576 字节（Kconfig LV_MEM_SIZE_KILOBYTES=24，
     * tlsf 开销后可用区实测约 21680）—— 该模式的用途就是复现真机约束。
     * 可用 -DLV_MEM_SIZE=... 覆盖。 */
    #ifndef LV_MEM_SIZE
...
/* 打开 stdio 文件系统驱动：模拟器靠它按路径读中文字体文件。
 * 走 LVGL 自己的 FS 接口（而不是 FreeType 直接 open()），与真机
 * （从 flash/SD 读字库）是同一套路径，模拟得更真。 */
#define LV_USE_FS_STDIO 1
#if LV_USE_FS_STDIO
    #define LV_FS_STDIO_LETTER 'A'      /**< 盘符 A: */
    #define LV_FS_STDIO_PATH ""         /**< 空 = 直接用传入的路径 */
    #define LV_FS_STDIO_CACHE_SIZE 4096 /**< 缓存，字库按需读取 */
#endif
...
/** FreeType library */
/* FreeType：模拟器用它直接加载 OTF/TTF 渲染中文（真机走编译进 flash 的静态字库）。
 * LVGL_PORT=1 让 FreeType 使用 LVGL 的文件/内存接口，于是会经过上面的 FS_STDIO。 */
#define LV_USE_FREETYPE 1
#if LV_USE_FREETYPE
    /** Let FreeType use LVGL memory and file porting */
    /* 0 = FreeType 用自己的文件读取（普通路径）。
     * 不能设 1：那会编译 lv_ftsystem.c，它需要 freetype/internal/*.h，
     * 而 Homebrew 的 freetype 只装公开头文件。真机走静态字库，用不到这条路。 */
...
#define LV_USE_SDL              1
    ...
    #define LV_SDL_DIRECT_EXIT      0    /**< 0: 退出交给我们的主循环控制 */
