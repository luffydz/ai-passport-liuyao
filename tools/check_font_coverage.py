...
背景（这个坑已经踩过一次）
    字库是按"实际用到的字符"裁剪的，分五档：
        kai_48 / kai_34  → 只给欢迎页大标题 / 卦名与数字
        sans_18          → 正文，含全部卦理数据（卦名/卦辞/爻辞/白话/建议…）
        sans_16 / sans_14→ 提示条与次要说明，**字符集小得多**
    如果一个字符串用 16px 渲染，但它的字只收进了 18px 那一档，真机就会显示
    方块 —— 而模拟器用 FreeType 动态加载，完全看不出来。
    实测就是这么翻车的：断卦页的"本卦 xxx / 变卦 xxx"两行从 18px 改成 16px
    之后，白话里的字不在 16px 字库里，真机变方块。

做法
    取出 16px 字库的字符集（它是两档小字库的上界），再把 ui/*.c 与 hexagram.c
    里所有字符串字面量逐个比对，列出**不在其中**的字符串。
...
def main() -> int:
    ...
    files = sorted(UI.glob("*.c")) + sorted(UI.glob("*.h"))
    risky = []
    for p in files:
