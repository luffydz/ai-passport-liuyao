// fw/main/main.c —— AI Passport · 六爻 App 的固件入口。
//
// 本文件只做两件事，刻意保持很薄：
...
//
// 一一对应关系（三个事件同名同义）：
//   BSP_BTN_PRESS → APP_KEY_PRESS   按下瞬间（摇卦"蓄力"靠它，必须转发）
...
void app_main(void)
{
    ESP_LOGI(TAG, "AI Passport · 六爻 启动");
    ...
    // NVS：存起卦日期用（真机没有 RTC，日期只能靠用户输入并记住）
    ...
    if (nvs_ret != ESP_OK) {
        ESP_LOGW(TAG, "NVS 初始化失败，起卦日期将无法保存");
    }
