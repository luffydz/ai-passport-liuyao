...
//
// 这一层是三行转调，但有两处必须小心：
//   1. 音频必须异步 —— bsp_audio_write() 是阻塞的，1.3 秒的铜钱声足以卡死
...
void app_port_impl_audio_start(void)
{
    ...
    if (xTaskCreate(audio_worker, "liuyao_audio", 4096, NULL, 5, NULL) != pdPASS) {
...
// ---------------------------------------------------------------------------
// 起卦日期持久化：写 NVS。
// 真机没有 RTC（断电即丢时间），六爻要的"今天"只能靠用户输入并记住。
// 打包成一个 int 存取：y*10000 + m*100 + d。
// ---------------------------------------------------------------------------
#define DATE_NVS_NS  "liuyao"
