...
#   cd "/Users/luffydz/Desktop/ai passport/每日穿衣/fw" && ./build.sh
set -euo pipefail
cd "$(dirname "$0")"
...
# 我们交付的刷机包：名字里带 apps，便于与基线固件区分
FULL_BIN="FoloToy-AI-Passport-apps-full.bin"

idf.py build
idf.py merge-bin -o "$PWD/build/$FULL_BIN"

# 官方校验脚本内部写死了合并包名 FoloToy-AI-Passport-full.bin，
# 而我们的包叫 ...-apps-full.bin。为了继续让官方脚本把守
...
# 所以从 每日穿衣/fw 往上是两层。本仓库不包含该脚本 —— 新克隆者没有它时跳过校验，
...
ARCHIVE_FILE="$ARCHIVE_DIR/FoloToy-AI-Passport-apps-$STAMP.bin"
